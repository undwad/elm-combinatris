	...
	function (styles, weights, lang) {
		...
		return A2(
			...
			elm$core$Platform$Cmd$none,
			{
				...
				rows: A2(elm$core$Array$repeat, height, author$project$Game$Game$makeRow),
				score: 0,
...
var author$project$Game$Game$makeCurr = F2(
	function (model, term) {
		...
		var info = function () {
			if (term.$ === 'Comb') {
				var comb = term.b;
				var _n1 = A2(elm$core$Dict$get, comb, model.lang);
				if (_n1.$ === 'Just') {
					var decl = _n1.a;
					return A2(
						author$project$Game$View$viewString,
...
var author$project$Game$Game$updateRow = F2(
	function (model, row) {
		...
		var score = row.score + (A2(elm$core$Basics$max, 0, row.width - width) * author$project$Game$Game$scoreFactor(model));
		return _Utils_update(
			row,
			{
		...
		switch (msg.$) {
			...
			default:
				...
				var model2 = _Utils_update(
					model1,
					{
						interval: model.interval - ((model.interval / author$project$Game$Game$scoreFactor(model)) / 1000),
			...
			[
				...
				_List_fromArray(
					[
						...
						elm$html$Html$Attributes$style,
						'font-size',
						'calc(100wh / ' + (elm$core$String$fromFloat(model.width * 1.1) + ')')),
						A2(elm$html$Html$Attributes$style, 'white-space', 'pre')
					]),
				_List_fromArray(
					[
