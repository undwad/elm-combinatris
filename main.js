...
{
	if (region.bG.W === region.aE.W)
	{
		return 'on line ' + region.bG.W;
	}
	return 'on lines ' + region.bG.W + ' through ' + region.aE.W;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { bn: 'hidden', S: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { bn: 'mozHidden', S: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { bn: 'msHidden', S: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { bn: 'webkitHidden', S: 'webkitvisibilitychange' }
		: { bn: 'hidden', S: 'visibilitychange' };
}
...
var author$project$Editor$Editor$init = function (_n0) {
	...
	return A2(
		...
		author$project$Editor$Port$readSlotNum,
		{
			k: _Utils_update(
				model,
				{T: 'exprarea', an: 1, aj: false, as: true}),
			l: elm$core$Result$Ok(_List_Nil),
			A: _Utils_update(
				model,
				{T: 'langarea', an: 10, aj: true, ak: author$project$Editor$Data$placeholder}),
...
var author$project$Main$init = F3(
	function (_n0, url, key) {
		...
		return A2(
			...
			{x: editor, U: game, ap: key, aS: url, D: 0},
...
var elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {aI: fragment, aL: host, aU: path, aW: port_, aZ: protocol, a_: query};
	});
	...
	function (msg, model) {
		if (!msg.$) {
			...
			return _Utils_update(
				model,
				{
					F: A2(author$project$Editor$CodeArea$cutLines, model.an, code)
				});
		} else {
			var scroll = msg.a;
			return _Utils_update(
...
var elm$regex$Regex$Match = F4(
	function (match, index, number, submatches) {
		return {bp: index, bt: match, bx: number, bI: submatches};
	});
...
var elm$parser$Parser$variable = function (i) {
	return elm$parser$Parser$Advanced$variable(
		{aG: elm$parser$Parser$ExpectingVariable, br: i.br, bD: i.bD, bG: i.bG});
};
...
var author$project$Game$Game$makeCurr = F2(
	function (model, term) {
		...
		var decl = function () {
			if (!term.$) {
				var comb = term.b;
				var _n1 = A2(elm$core$Dict$get, comb, model.z);
				if (!_n1.$) {
					var decl1 = _n1.a;
					return A2(
						author$project$Game$View$viewString,
						model.ac,
						author$project$Lang$Show$showDecl(decl1));
				} else {
					return _List_Nil;
				}
			} else {
		...
		return {
			...
			P: 0,
			w: elm$core$Basics$floor(model.ag / 2)
		};
	});
	...
	function (model, row) {
		...
		var expr = A2(
			...
			A3(
				...
				code));
		var offsets = author$project$Lang$Game$canInsertTerm(expr) ? _List_Nil : A2(author$project$Lang$Lang$getReductionOffsets, model.z, expr);
...
var author$project$Game$Game$prependRow = F3(
	function (term, model, row) {
		var expr = A3(author$project$Lang$Game$prependTerm, term, model.z, row.t);
		return A2(
...
var author$project$Game$Game$reduceRow = F2(
	function (model, row) {
		var expr = row.az ? A2(author$project$Lang$Game$reduceLeft, model.z, row.t) : row.t;
		return A2(
			author$project$Game$Game$updateRow,
			model,
			_Utils_update(
	...
	function (msg, model) {
		...
		var control = F2(
			function (dy, m) {
				...
				if (!_n6.$) {
					var curr = _n6.a;
					var _n7 = A3(canMove, curr.P + curr.i, curr.w + dy, m.m);
					if (_n7) {
						return _Utils_update(
							m,
							{
		...
		var move = F2(
			function (dx, m) {
				...
				if (!_n4.$) {
					var curr = _n4.a;
					var _n5 = A3(canMove, (curr.P + dx) + curr.i, curr.w, m.m);
					if (_n5) {
						return _Utils_update(
							m,
							{
						...
						return _Utils_update(
							...
							{
								s: elm$core$Maybe$Nothing,
								m: A3(prepend, curr.a8, curr.w, m.m)
							});
					}
				} else {
					return _Utils_update(
		...
		switch (msg.$) {
			case 0:
				return A2(author$project$Misc$perform, elm$core$Platform$Cmd$none, model);
			case 1:
				return A3(author$project$Game$Game$init, model.ac.al, model.av, model.z);
			case 2:
				return A2(
					author$project$Misc$perform,
					elm$core$Platform$Cmd$none,
				...
				case 1:
					if (!_n0.b) {
						...
						return A2(
							author$project$Main$mapEditor,
							model,
							A2(author$project$Editor$Editor$update, msg1, model.x));
					} else {
						break _n0$5;
					}
				case 2:
									...
									case 'NEW':
										return A2(
											...
											A2(
												...
												model));
									case 'game':
										return author$project$Editor$Utils$isLangStyled(model.x) ? author$project$Main$startGame(model) : A2(
											author$project$Misc$perform,
											author$project$Main$resetUrl(model),
											model);
									default:
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				(elm$core$String$isEmpty(model.F) && (!elm$core$String$isEmpty(model.ak))) ? author$project$Editor$CodeArea$viewPlaceholderArea(model) : author$project$Editor$CodeArea$viewCodeArea(model)
			]));
};
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										A2(author$project$Game$View$coloredText, '#bdc3c7', 'Rule: ')
									])),
								A2(
								elm$html$Html$td,
								_List_Nil,
